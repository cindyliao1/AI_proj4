    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                allPossible[p]=emissionModel[true_dist]*self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            old_B = self.getBeliefDistribution()
            ...
            for position in legal_positions:
                ...
                possible[position] = emissionModel[distance] * old_B[position]
            if not any(possible.values()):
                self.initializeUniformly(gameState)
            else:
                t = []
                for i in range(0, self.numParticles):
                    t.append(util.sample(possible))
...
class JointParticleFilter:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newPos = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
                newParticle[i] = util.sample(newPos)
            "*** END YOUR CODE HERE ***"
